...
test.describe('Fake Persona Generation', () => {
  ...
  async function expectNameGenderResults(page, scope = page) {
    await expect(scope.locator('.firstNameValue')).toHaveText(/[^\s]/, { useInnerText: true });
    await expect(scope.locator('.lastNameValue')).toHaveText(/[^\s]/, { useInnerText: true });
    await expect(scope.locator('.genderValue')).toHaveText(/^(male|female)$/i, { useInnerText: true });
  }

  async function expectCpr(page, scope = page) {
    await expect(scope.locator('.cprValue')).toHaveText(/[^\s]/, { useInnerText: true });
  }

  async function expectDob(page, scope = page) {
    await expect(scope.locator('.dobValue')).toHaveText(/[^\s]/, { useInnerText: true });
  }

  async function expectStreet(page, scope = page) {
    await expect(scope.locator('.streetValue')).toHaveText(/[^\s]/, { useInnerText: true });
  }

  async function expectTown(page, scope = page) {
    await expect(scope.locator('.townValue')).toHaveText(/[^\s]/, { useInnerText: true });
  }

  async function expectPhone(page, scope = page) {
    await expect(scope.locator('.phoneNumberValue')).toHaveText(/[^\s]/, { useInnerText: true });
  }
